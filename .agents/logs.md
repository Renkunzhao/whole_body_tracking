...
- `2026-05-25`：整理 trampoline ball-drop 对照 6 次运行。仿真器：IsaacLab 4 次、MuJoCo 2 次。输出目录：`logs/isaaclab_trampoline_ball_drop_runs/20260525_171542__sweep-old-lowdamp/`、`logs/isaaclab_trampoline_ball_drop_runs/20260525_171713__sweep-old-highdamp/`、`logs/isaaclab_trampoline_ball_drop_runs/20260525_172606__sweep-new-mass 10/`、`logs/mujoco_ball_drop_runs/20260525_172809__mujoco_nominal__t4__bm4.02__h1__m10__r0.03__sp1.5__x0__solref0.01-1-align/`、`logs/isaaclab_trampoline_ball_drop_runs/20260525_172419__sweep-new-mass 100/`、`logs/mujoco_ball_drop_runs/20260525_172743__mujoco_nominal__t4__bm4.02__h1__m100__r0.03__sp1.5__x0__solref0.01-1-align/`。全部 stable，无 fallthrough/off_edge。

  | 组 | 运行 | 仿真器 | 关键参数 | max compression (m) | first min z @ t (m, s) | first apex h @ t (m, s) | second apex h (m) | stable comp @ t (m, s) |
  |---|---|---|---|---:|---:|---:|---:|---:|
  | old | old-lowdamp | IsaacLab | `th=0.1, mass=5, res=10, E=8e4, edamp=0.01, dscale=1.0` | 0.1703 | -0.1189 @ 0.512 | 0.2731 @ 0.820 | 0.0274 | 0.0164 @ 1.786 |
  | old | old-highdamp | IsaacLab | `th=0.1, mass=5, res=10, E=8e4, edamp=0.1, dscale=1.0` | 0.1884 | -0.1161 @ 0.532 | -0.0183 @ 1.168 | nan | 0.0450 @ 1.026 |
  | new mass=10 | IsaacLab candidate | IsaacLab | `th=0.05, mass=7.5, res=15, E=1.5e5, edamp=0.005, dscale=0.5` | 0.1826 | -0.1563 @ 0.540 | 0.1570 @ 0.826 | 0.0128 | 0.0414 @ 1.838 |
  | new mass=10 | MuJoCo target | MuJoCo | `mass=10, radius=0.03, spacing=1.5, solref=0.01 1` | 0.1880 | -0.1769 @ 0.518 | 0.1415 @ 0.791 | -0.0117 | 0.0810 @ 2.587 |
  | new mass=100 | IsaacLab candidate | IsaacLab | `th=0.05, mass=20, res=15, E=3e5, edamp=0.005, dscale=0.5` | 0.0967 | -0.0714 @ 0.536 | 0.0410 @ 0.708 | -0.0056 | 0.0312 @ 1.188 |
  | new mass=100 | MuJoCo target | MuJoCo | `mass=100, radius=0.03, spacing=1.5, solref=0.01 1` | 0.0871 | -0.0820 @ 0.499 | -0.0055 @ 0.662 | -0.0233 | 0.0539 @ 1.516 |

  结论：old-lowdamp 回弹过高且稳态下陷太浅；old-highdamp 压缩接近但耗能过强、几乎无有效回弹。new mass=10 的 IsaacLab candidate 与 MuJoCo mass=10 在动态压缩和第一 apex 上接近，但稳态下陷仍偏浅；new mass=100 的 IsaacLab candidate 与 MuJoCo mass=100 在动态压缩上接近，但回弹略高、稳态下陷偏浅。

- `2026-05-25`：更新 IsaacLab ball-drop sweep 输出逻辑。脚本：`scripts/isaaclab_trampoline_ball_drop.py`。变更：sweep 全部子运行结束后，除原有 `ball_drop_sweep_summary.csv` 外，自动生成 `ball_drop_sweep_analysis.csv` 和 `ball_drop_sweep_first_min_apex.png`；横轴自动选择变化的 sweep 参数，优先 `trampoline_mass/ball_mass/youngs_modulus/elasticity_damping/damping_scale`，纵轴为 `first_min_ball_z_m`（主指标）和 `first_apex_height_m`。目的：在不依赖 top-center compression 的情况下，用第一次压缩最低点和第一次 apex 高度做超参数分析。已运行 `python -m py_compile scripts/isaaclab_trampoline_ball_drop.py` 通过。

- `2026-05-25`：更新 IsaacLab ball-drop sweep 分析图。脚本：`scripts/isaaclab_trampoline_ball_drop.py`。变更：新增 `--sweep_xscale {linear,log}`，也可在 sweep YAML 顶层设置 `sweep_xscale: log`；`ball_drop_sweep_first_min_apex.png` 的横轴缩放由该参数显式控制，默认 `linear`，不再按 `youngs_modulus` 名称隐式切换。目的：Young's modulus 或 damping 跨数量级扫描时可手动使用 log 横轴，同时保持最简、显式配置。已运行 `python -m py_compile scripts/isaaclab_trampoline_ball_drop.py` 通过。
