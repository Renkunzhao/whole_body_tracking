...
$\bm{\omega}^{b}_t$ is base angular velocity. The actor observation uses
additive noise on root position, base linear velocity, base angular velocity,
joint position, and joint velocity. The critic uses the same observation terms
without corruption. The policy action is a 12-DoF joint-position target action
