...
\paragraph{Privileged trampoline-parameter teacher observation.}
For teacher-policy and RMA-style experiments, a privileged actor observation
variant appends the true randomized trampoline parameters:
\[
    \bm{o}^{\mathrm{tramp}} =
    \left[
        \log(E/E_0),\,
    ...
    \right].
\]
This term is used only by teacher tasks and is not part of the deployable actor
observation. Its purpose is to test the upper-bound performance of a policy
that knows the trampoline dynamics, and to provide a target for later latent
dynamics estimation.
