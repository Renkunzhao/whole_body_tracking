...
def reset_drop_from_height(
    ...
    """Reset the robot dropped from ``h_cmd`` for the single-bounce task.

    Samples ``h_cmd ~ U(cmd.cfg.ranges.peak_height)`` per env, writes it into
    the rebounce command's target buffer, and teleports the robot to
    ``(x_default + env_origin, y_default + env_origin, drop_z_offset + h_cmd)``
    with default orientation, zero linear/angular velocity, and default joint
    state. Because event-mode resets run before ``command_manager.reset()``,
    this is the single place that samples ``h_cmd`` per episode — the command
    term's ``_resample_command`` is intentionally a no-op.

    Args:
        drop_z_offset: Added to ``h_cmd`` to set the absolute base z. Use 0
            for "drop_z = h_cmd in world frame", or e.g. the robot's standing
            height for "drop_z = h_cmd above standing pose".
    """
    ...
    root_state[:, 2] = env.scene.env_origins[env_ids, 2] + drop_z_offset + h_cmd
