...
def attach_onnx_metadata(env: ManagerBasedRLEnv | DirectRLEnv, run_path: str, path: str, filename="policy.onnx") -> None:
    onnx_path = os.path.join(path, filename)

    if isinstance(env, DirectRLEnv):
        print("Warning: Attaching ONNX metadata is skipped because the environment is not ManagerBasedRLEnv.")
        return
